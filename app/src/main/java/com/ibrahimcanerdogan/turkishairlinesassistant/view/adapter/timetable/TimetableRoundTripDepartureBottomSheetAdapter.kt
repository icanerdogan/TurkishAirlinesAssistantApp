...
class TimetableRoundTripDepartureBottomSheetAdapter(
    private val timetableResponse: ArrayList<OriginDestinationOption>
) : RecyclerView.Adapter<TimetableRoundTripDepartureBottomSheetAdapter.ViewHolder>() {
    ...
    override fun getItemCount(): Int {
        return timetableResponse.size
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        with(holder) {
            val sortedList = timetableResponse.sortedBy { it.FlightSegment.DepartureDateTime.split("T")[0] }
